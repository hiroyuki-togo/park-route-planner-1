...
**最終更新**: 2026-05-21（当日モードの実運用対応 完了後）
**来園日**: 2026-05-25（月）
**残り日数**: 4 日
...
**Phase 1〜6 完了 + 仕様追加（シミュレーションモード）完了 + 仕様追加（当日モードの実運用対応）完了**。
32 コミット、**テスト 55/55 PASS**、Streamlit 起動確認済み。
シミュレーションと当日モードの役割分担が明確化された（シミュ = 前日叩き台、当日 = リアルタイム再生成）。
残るは Phase 7（デプロイ）と、東郷さん側での **当日モード新 UI の目視確認**（§3 末尾チェックリスト）。
...
| T-D5 | 動作確認 + ドキュメント更新 | （本コミット） |

**動作確認の残タスク**（東郷さん側）:

- [ ] 当日モードを開くと、雨天モード直下に「現在時刻」「現在位置」が並ぶ
- [ ] 「現在時刻」のデフォルトが現在分、「現在位置」のデフォルトが「エントランス」
- [ ] アトラクション設定 expander の各行が `[必ず乗る][乗った][優先度 slider]` の 3 列
- [ ] 「乗った」を ON → 該当アトラクションが result.steps に出ない
- [ ] 「乗った」+「必ず乗る」両方 ON → 既消化扱い、unvisited_musts に入らない
- [ ] 「現在位置」= pooh、「現在時刻」= 13:00 でルート生成 → 最初の step の arrive が 13:00 以降、travel_min が pooh からの距離（≒ 小さい）
- [ ] 🧹 セッションリセット → 「乗った」が全部外れる、「現在時刻」が現在に戻る、「現在位置」がエントランスに戻る
- [ ] localStorage に `visited_attractions` が保存されている（DevTools）
- [ ] シミュレーションモードでは「現在時刻」「現在位置」が非表示、各行は `[必ず乗る][優先度 slider]` の 2 列
- [ ] シミュレーションモードでルート生成 → 既存挙動と同じ（9:00 開園想定 / エントランス起点 / 乗った無視）
...
シミュ・当日モードの役割整理が終わり、機能的にはほぼ完成形。優先順位の候補：

1. **Phase 7 デプロイ**（プラン Task 26-28）— `requirements.txt` 生成、README、GitHub リポジトリ作成、Streamlit Community Cloud デプロイ、統合テスト
2. **当日モード新 UI の目視確認**（§2 末尾チェックリスト）→ 不具合があれば優先で修正
3. **§3 仕様見直し論点 A〜D の続き**（A1 スコア式、B1 DPA 警告位置、B5 unvisited_musts の名前表示、等）
