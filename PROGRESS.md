...
**最終更新**: 2026-05-21（当日モードの実運用対応 + 過去ブロック修正 完了後）
**来園日**: 2026-05-25（月）
**残り日数**: 4 日
...
**Phase 1〜6 完了 + シミュレーションモード + 当日モード実運用対応 完了**。
34 コミット、**テスト 57/57 PASS**、Streamlit 起動確認済み + 東郷さん目視確認済み。
シミュレーションと当日モードの役割分担が明確化（シミュ = 前日叩き台、当日 = リアルタイム再生成）。
過去ブロックがルートに混入するバグも修正済み。残るは Phase 7（デプロイ）と、**デザイン変更（CSS適用）** + **未検討事項の整理**。
...
### 🟢 次回のモード：**未検討事項の整理 + デザイン変更（CSS適用）**

東郷さんからの指定：「次回は、未検討事項とデザイン変更（CSS適用）を進めたい」。
Phase 7 デプロイは仕様面が固まってから着手する想定。来園日まで 4 日 + 機能はほぼ完成しているのでスケジュール上問題なし。

#### A. 次回扱う「未検討事項」候補リスト

詳細な現状と検討案は **下記「§3 末尾：仕様見直しの候補リスト」** を参照。次セッション冒頭で優先順位を東郷さんと決める。

**従来からの論点（A〜D）**
- A1 / A2：スコア式、priority=0 と must-visit の競合
- B1〜B5：DPA 警告位置、並び順、ルート表示、食事初期値、unvisited_musts の名前表示
- C1 / C2：予測精度（過剰実装しない）、`get_time_factor(21)` のフォールスルー
- D1 / D2：ダミー snapshot 運用、`requires_reservation` の再確認（5/24 リハ）

**今回セッションで気づいた追加論点（v2 候補含む）**
- **E1**. ルート結果の各 step に「✓ 消化済み」ボタン（B 案として今回見送り）
- **E2**. 「乗った」リストの一括クリアボタン（セッション/完全リセットで足りているが運用次第）
- **E3**. 当日モードで「現在時刻」を毎回修正させる UX → 「いま」ボタンが欲しいかも
- **E4**. シミュレーションモードでも「途中入園」シナリオに対応するか

#### B. デザイン変更（CSS適用）

Streamlit で CSS を適用する手段は主に 3 つ：
1. `st.markdown("<style>...</style>", unsafe_allow_html=True)` で global CSS を inject
2. `st.html(...)` で個別の HTML/CSS ブロックを描画
3. `.streamlit/config.toml` のテーマ設定（色・フォントサイズ程度なら）

次回の冒頭で東郷さんが「どこをどう変えたいか」のラフ要望を共有 → 私が「Streamlit でできる範囲・できない範囲」を整理 → 優先順位を決めて適用、の流れを想定。

#### C. Phase 7（デプロイ）

未検討事項と CSS が片付いたら着手。プラン Task 26-28：
...
- 5/20 水 → **Phase 5 + 6 完了**
- 5/21 木 → **シミュレーションモード追加 + リセット機能 + 当日モードの実運用対応 完了**
...
```
ディズニーランドのルート生成ツール、機能面はほぼ完成（テスト 57/57 PASS）。
今日は「未検討事項の整理」と「デザイン変更（CSS適用）」を進めたい。

まず PROGRESS.md §3 A の論点（A1〜A2 / B1〜B5 / C1〜C2 / D1〜D2 / E1〜E4）を読んで、それぞれの現状＋検討すべき軸を一覧化してください。優先順位は私が決める。

CSS 適用については、Streamlit でできる範囲（st.markdown + unsafe_allow_html / st.html / config.toml）の整理から始めてほしい。私の要望（どこをどう変えたいか）は次に共有します。

事前に `.venv/bin/pytest -q` で 57/57 PASS を確認し、`.venv/bin/streamlit run app.py` で UI が起動することも見ておいてください。
```

### 引き継ぎチェックリスト（次セッション冒頭で確認すること）

- [ ] `git log --oneline -10` で最近のコミット（`6c49c5a` 過去ブロック修正、`183061c / 2874f71 / 46c6b8b` 当日モード対応、`25f7f23 / 432c4ba` リセット、`3646ecf / 7dbe760 / 33511bb / 1eef2a1` シミュ）が見えること
- [ ] `git status` でクリーン
- [ ] `.venv/bin/pytest -q` で **57 passed**
- [ ] `.venv/bin/streamlit run app.py` で UI が起動し、シミュ/当日両モードのトグル・新 UI が表示されること
- [ ] CLAUDE.md / lessons.md（#17〜#19 を含む）/ PROGRESS.md §3 を読んで論点を把握
